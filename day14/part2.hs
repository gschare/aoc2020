...
applyMask ((i,b):rest) x =
    case b of
      ...
      0 -> applyMask rest x
      -1 -> applyMask rest (x `setBit` i) ++ applyMask rest (x `clearBit` i)

memSet :: Int -> Int -> Memory -> (Int -> [Int]) -> Memory
memSet k v mem f = foldl (\acc x -> M.insert x v acc) mem (f k)
...
parseMask xs = map (\(a,b) -> (a, if b == 'X' then (-1) else read [b])) $ zip [length xs - 1, length xs - 2..0] xs
