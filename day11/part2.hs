...
findNearestNeighbor :: Coord -> Coord -> Grid -> Maybe Cell
findNearestNeighbor (i,j) (di,dj) g =
    case getCell (i',j') g of
      Nothing -> Nothing
      Just Floor -> findNearestNeighbor (i',j') (di,dj) g
      Just x -> Just x
    where i' = i + di
          j' = j + dj

countFullNeighbors :: Coord -> Grid -> Int
countFullNeighbors c g = sum fullNeighbors
    where neighbors = map (\dc -> findNearestNeighbor c dc g) offsetMatrix
          f Nothing = 0
          f (Just Full) = 1
          f _ = 0
...
progressCell (coord,state) g
    ...
    | state == Full  && fullNeighbors >= 5 = Empty
    | otherwise = state
